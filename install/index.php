...
switch ($_POST['step'])
{
	default :
		...
		if (is_really_writable(PUB_PATH . 'cache'.DIRECTORY_SEPARATOR) OR defined('IN_SAE'))
		{
			$system_require['config_writable_cache'] = TRUE;
		}
		if (is_really_writable(PUB_PATH . 'tmp'.DIRECTORY_SEPARATOR) OR defined('IN_SAE'))
		{
			$system_require['config_writable_tmp'] = TRUE;
		}
		if (is_really_writable(PUB_PATH . 'uploads'.DIRECTORY_SEPARATOR) OR defined('IN_SAE'))
		{
	...
	case 2 :
		if (!defined('IN_SAE'))
		{
			...
			{
				if (! is_dir(PUB_PATH . $dir_name))
				{
					if (! @mkdir(PUB_PATH . $dir_name))
					{
						$error_messages[] = load_class('core_lang')->_t('目录: %s 无法创建，请将网站根目录权限设置为 777, 或者创建这个目录设置权限为 777', PUB_PATH . $dir_name);
					}
				}
			}
	...
	case 3 :
		...
		if (sprintf("%2f", $db->getServerVersion()) < 5.3)
		{
		...
		// 创建数据表
		$db_table_querys = explode(";\r", str_replace(array('[#DB_PREFIX#]', '[#DB_ENGINE#]', "\n"), array($_POST['db_prefix'], $_POST['db_engine'], "\r"), file_get_contents(PUB_PATH . 'install/db/mysql.sql')));
	...
	case 4 :
		...
		$insert_query = file_get_contents(PUB_PATH . 'install/db/system_setting.sql');
		...
		else
		{
			...
			$insert_query = str_replace('[#UPLOAD_DIR#]', serialize(str_replace("\\", "/", PUB_PATH) . "uploads"), $insert_query);
		}
		...
		$db->insert($db_prefix . 'system_setting', array(
			...
			'value' => serialize(file_get_contents(PUB_PATH . 'install/db/register_agreement.txt')),
		));
