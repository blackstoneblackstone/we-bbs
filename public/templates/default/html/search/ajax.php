...
<dl>
    <dt class="mb-1">
        ...
        <a href="javascript:;" class="aw-username">
            <img src="/static/common/image/default-avatar.svg" class="aw-user-img circle" alt="匿名用户">匿名用户
        </a>
        {else/}
...
<dl class="d-flex position-relative">
    ...
    <dd class="flex-fill w-100 mb-0">
        ...
        <p class="aw-one-line">{$v['signature']|default='这家伙还没有留下自我介绍～'|raw}</p>
    </dd>
    ...
    <dd class="position-absolute" style="right: 0;top:10px">
        <a href="javascript:;" class="mr-3 text-primary {$v['has_focus'] ? 'active ygz' : ''}" onclick="AWS.User.focus(this,'user','{$v.uid}')">
            {$v['has_focus'] ? '<span><i class="icon-minus-circle text-danger"></i> 已关注</span>' : '<span><i class="icon-plus-circle text-primary"></i> 关注</span>'}
        </a>
        <a href="javascript:;" class="aw-send-inbox text-smooth">私信</a>
    </dd>
    {/if}
</dl>
{/if}
...
<dl class="topic position-relative clearfix d-flex">
    ...
    <dd class="flex-fill" style="width: calc(100% - 75px);">
        ...
        <p class="font-8 text-muted">
            <span class="mr-3">正在讨论：{$v.discuss}</span>
            <span class="mr-3"><span class="aw-global-focus-count">关注人数：{$v.focus}</span></span>
            {if $user_id}
            <a href="javascript:;" class="cursor-pointer {$v['has_focus'] ? 'active ygz' : ''}" onclick="AWS.User.focus(this,'topic','{$v.id}')" >{$v['has_focus'] ? '<span><i class="icon-minus-circle text-danger"></i> 已关注</span>' : '<span><i class="icon-plus-circle text-primary"></i> 关注</span>'}</a>
            {/if}
        </p>
    </dd>
</dl>
...
<dl>
    ...
    <dd>
        <label class="dz">
            <a type="button" class="btn btn-primary btn-sm aw-ajax-agree  {$v['answer_info']['vote_value']==1 ? 'active' : ''}" title="点赞回答" onclick="AWS.User.agree(this,'answer','{$v['answer_info']['id']}');"><i class="iconfont">&#xe870;</i>赞同<span class="badge">{$v['answer_info']['agree_count']}</span></a>
            <a type="button" class="btn btn-primary btn-sm aw-ajax-against  {$v['answer_info']['vote_value']==-1 ? 'active' : ''}" title="反对回答" onclick="AWS.User.against(this,'answer','{$v['answer_info']['id']}');"><i class="iconfont">&#xe68f;</i>反对<span class="badge"></span></a>
        </label>
        <label class="ml-3 mr-3"><i class="iconfont">&#xe625;</i>{$v['answer_info']['thanks_count']}人感谢</label>
        <label class="mr-3"><i class="iconfont">&#xe601;</i>{$v['answer_info']['comment_count']}评论</label>
    </dd>
</dl>
{/if}
