...
<div class="bg-white p-3 mb-1 aw-article-comment-item" data-total="{$total}" id="article-comment-{$v.id}">
	...
	<div class="actions">
		<div class="font-9 mt-2">
            <a href="javascript:;" class="text-muted aw-ajax-agree mr-3 {if $v['vote_value']==1}active{/if}" onclick="AWS.User.agree(this,'article_comment','{$v.id}')"><i class="icon-thumb_up"></i> 点赞 <span>{$v.agree_count}</span></a>
			{if $user_id}
			...
			<a href="javascript:;" class="text-muted aw-ajax-get" data-confirm="确定要删除吗？" data-url="{:url('comment/remove_article_comment',['id'=>$v.id])}"> <i class="icon-delete mr-1"></i>删除 </a>
            {/if}
            {/if}
		</div>
	</div>
