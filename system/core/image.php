...
class core_image
{
    const IMAGE_CORE_OP_TO_FILE=1;
    const IMAGE_CORE_OP_OUTPUT=2;
    const IMAGE_CORE_SC_NOT_KEEP_SCALE=4;
    const IMAGE_CORE_SC_BEST_RESIZE_WIDTH=8;
    const IMAGE_CORE_SC_BEST_RESIZE_HEIGHT=16;
    const IMAGE_CORE_CM_DEFAULT=0;
    const IMAGE_CORE_CM_LEFT_OR_TOP=1;
    const IMAGE_CORE_CM_MIDDLE=2;
    const IMAGE_CORE_CM_RIGHT_OR_BOTTOM=3;
	...
	var $height;
	var $quality = 90;
	var $option = self::IMAGE_CORE_OP_TO_FILE;
	var $scale;
	// clipping method 0: default 1: left or top 2: middle 3: right or bottom
	var $clipping = self::IMAGE_CORE_CM_MIDDLE;
	...
	private function imageProcessImageMagick()
	{
		...
		if ($this->clipping != self::IMAGE_CORE_CM_DEFAULT)
		{
		...
		if ($this->scale != self::IMAGE_CORE_SC_NOT_KEEP_SCALE)
		{
			if ($this->scale == self::IMAGE_CORE_SC_BEST_RESIZE_WIDTH)
			{
				$resize_height = round($this->width * $this->source_image_h / $this->source_image_w);
				$resize_width = $this->width;
			}
			else if ($this->scale == self::IMAGE_CORE_SC_BEST_RESIZE_HEIGHT)
			{
		...
		if ($this->option == self::IMAGE_CORE_OP_TO_FILE AND $this->new_image)
		{
			file_put_contents($this->new_image, $im->getimageblob());
		}
		else if ($this->option == self::IMAGE_CORE_OP_OUTPUT)
		{
	...
	private function imageProcessGD()
	{
		...
		if ($this->scale & self::IMAGE_CORE_SC_BEST_RESIZE_HEIGHT)
		{
		...
		else if ($this->scale != self::IMAGE_CORE_SC_NOT_KEEP_SCALE)
		{
			if ($this->source_image_w * $this->height > $this->source_image_h * $this->width)
			{
