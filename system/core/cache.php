...
class core_cache
{
	...
	public function __construct()
	{
        ...
        if(get_setting('cache_type') == 'redis')
        {
            ...
            $this->backendOptions = ['servers' =>AWS_APP::config()->get('cache')->Redis];
        }
        else if(get_setting('cache_type') == 'memcached')
        {
            $this->backendName = 'Memcached';
            $this->backendOptions = ['servers' =>AWS_APP::config()->get('cache')->Memcached];
        }
        else if(get_setting('cache_type') == 'memcache')
        {
            $this->backendName = 'Memcache';
            $this->backendOptions = ['servers' =>AWS_APP::config()->get('cache')->Memcache];
        }else
        {
