...
function fetch_ip()
{
    ...
    if (!valid_ip($ip) AND $_SERVER['REMOTE_ADDR'])
    {
...
function _show_error($exception_message)
{
	...
	if ($exception_message)
	{
		...
		$errorBlock = "<div style='display:block' id='exception_message'><textarea rows='15' onfocus='this.select()'>{$exception_message}</textarea></div>";
	}
...
function db($name='',$db_config=[],$driver='MySQLi',$prefix=null)
{
    try {
        $db = Db::instance($driver, $db_config, $prefix);
        if(!$name) return $db;
        return $db->name($name);
