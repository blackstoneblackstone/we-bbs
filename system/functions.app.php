 ...
 * @return mixed
 */
function hook(string $plugin, string $method, $params = [])
{
...
function remove_assoc($from, $type, $id)
{
	...
	return AWS_APP::model()->query('UPDATE ' . AWS_APP::model()->get_table($from) . ' SET `' . $type . '_id` = NULL WHERE `' . $type . '_id` = ' . $id);
}
    ...
    if ($strUrl) {  
        ...
        if (!empty($img)) {
            ...
            foreach($img as $imgItem){
                ...
                }elseif ((strstr($imgItem, 'http') || strstr($imgItem, 'https')) && !strstr($imgItem, base_url())){
                    $replacements[] = $imgItem;
                    $img_new = "/".preg_replace("/\//i","\/",$imgItem)."/";
                    $patterns[] = $img_new;
                }
            ...
            return $vote_content ? $vote_content : $content;
        }else {  
            return $content;  
        }                     
    } else {  
...
{
	return json_decode(str_replace(['$title','$content','$time','$answer_content'],[$title,$content,$time,$answer_content],$data),true);
}
...
function remove_xss($html,$remove=false)
{
	...
	if(get_setting('enable_remove_xss')=='Y' || $remove)
	{
		...
		//允许的标签
		//$config->set('HTML.Allowed', 'div,b,strong,i,em,a[href|title],ul,ol,li,p[style],br,span[style],img[width|height|alt|src],script[onclick],video');
		$def = $config->getHTMLDefinition(true);
		//$def->addAttribute('a', 'onclick', 'CDATA');
		$def->addAttribute('a', 'download', 'CDATA');
		$def->addAttribute('a', 'encode', 'CDATA');
        $def->addAttribute('a', 'download-encrypt', 'CDATA');
		//可自定义过滤规则，适合二次开发使用
...
function RemoveXSS($val) {
    ...
    $ra1 = Array('javascript', 'vbscript', 'expression', 'applet', 'meta', 'xml', 'blink', 'link', 'script', 'ilayer', 'layer', 'bgsound', 'base');
 ...
 * @return bool|string
 */
function curl_request($url,$data = null,$json=0)
{
...
function replace_image($content,$type,$replace=false)
{
    ...
    if(get_setting('watermark_enable') == 'Y')
    {
    	$content = watermark_image($content,$replace);
    }
    return $content;
}
...
function sync_img($url, $type)
{
    try {
        ...
        $ext1 = explode(".",$url);
        $ext1 = end($ext1);
        $ext1 = in_array(strtolower($ext1),['jpg','png','gif','jpeg','bmp']) ? $ext1 : 'jpg';
        $ext = '.'.$ext1;

        if(get_hook_info('osd')['state']==1 and get_hook_config('osd')['group']['base']['config']['status']['value']!='no')
        {
            $ret=hook('osd','upload_file',['cat'=>$type,'file'=>$file,'ext'=>$ext]);
            if($ret['error'] )
            {
                return $url;
            }
            ...
            {
                return get_setting('upload_url').'/'.$type.'/' .  gmdate('Ymd').'/'.$upload_data['file_name'];
            }
            return  $url;
        }
    } catch (Exception $e) {
...
/*图片添加水印*/
function watermark_image($content,$replace=false)
{
    //匹配图片的src
    preg_match_all('#<img.*?src="([^"]*)"[^>]*>#i',$content, $match);
    foreach($match[1] as $img_url)
    {
        if((strstr($img_url, 'http') || strstr($img_url, 'https')) && !strstr($img_url, base_url()) && get_setting('down_img_to_location_enable')=='N' && !$replace)
        {
            continue;
        }else{
	        $water_file_path = AWS_APP::picture()->watermark($img_url,get_setting('watermark_type'));
	        $content = str_replace($img_url,$water_file_path,$content);
        }
    }
    return $content;
}

function file2base64($image_file){
    $base64_image = '';
...
// 自定义文件大小text
function formatFilesize($size, $delimiter = '')
{
