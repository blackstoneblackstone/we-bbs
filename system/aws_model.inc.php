...
class AWS_MODEL
{
	...
	public function insert_all($table, $data)
    {
        ...
        {
            $value_data[$k] = implode('\',\'',$v);
        }
        $sql = 'INSERT INTO `' . $this->get_table($table) . '` (' . implode(',',array_unique($column_data)) . ') VALUES (\'' . implode('\'), (\'', $value_data) . '\')';
        if (AWS_APP::config()->get('system')->debug)
        {
            $start_time = microtime(TRUE);
        }
        ...
        return isset($rows_affected) ? $rows_affected : false;
    }
    ...
    public function query_install($sql, $limit = null, $offset = null, $where = null)
    {
        $this->slave();
