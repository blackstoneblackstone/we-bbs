...
class AWS_APP
{
	...
	public static function run()
	{
        ...
        //记录系统日志
        $notWriteApp = ['crond', 'admin'];
        $notWriteAct = ['notifications',];
        if(in_array('access',get_setting('log_remark_type')) && !in_array($_GET['app'],$notWriteApp) && !in_array($_GET['act'],$notWriteAct))
        {
            LOG::write('',null,'access');
        }
	...
	private static function init()
	{
		...
		if ((!defined('G_SESSION_SAVE') OR G_SESSION_SAVE == 'db') AND get_setting('db_version') > 20121123)
		{
			Zend_Session::setSaveHandler(new Zend_Session_SaveHandler_DbTable(array(
                ...
                'lifetime'=>AWS_APP::config()->get('session')->lifetime,
			)));
			self::$session_type = 'db';
		}

        if(G_SESSION_SAVE == 'redis')
        {
            Zend_Session::setSaveHandler(new Zend_Session_SaveHandler_Redis(AWS_APP::config()->get('session')->Redis));
		...
		self::$session = new Zend_Session_Namespace(G_COOKIE_PREFIX . '_WeCenter');
    ...
    {
        if(in_array('error',get_setting('log_remark_type')))
        {
            LOG::write($exception->getMessage(),null,'error');
        }
        if(get_setting('exception_handle_enable')!='Y' || get_setting('debug_enable')=='Y')
        {
     ...
     * @return void
     */
    public static function API($data, $code = 0, $msg = '')
    {
