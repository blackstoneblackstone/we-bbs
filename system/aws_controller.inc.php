	...
	public function __construct($process_setup = true)
	{
		...
		if (!in_array($_GET['app'], array('upgrade')))
		{
			...
			else
			{
				...
				if (AWS_APP::model('account')->get_user_info_by_uid($fromuid))
				{
					HTTP::set_cookie('fromuid', $fromuid);
				}
			}
			#用户删除
		...
		if (get_setting('site_close') == 'Y' AND $this->user_info['group_id'] != 1 AND !in_array($_GET['app'], array('admin', 'account', 'upgrade','m')) && !in_array($_GET['act'],['account']))
		{
			$this->model('account')->logout();
			$login_url = is_mobile() ? get_js_url('/m/account/login/') : get_js_url('/account/login/');
