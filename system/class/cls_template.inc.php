...
class TPL
{
	...
	{
		if (strpos($template_filename, self::$template_ext) === false)
		{
	...
	public static function import_css($path)
	{
		if (is_array($path))
		{
			...
			{
				if (strpos($val, 'css/') === 0 && strpos($val, '/admin/') === false)
				{
		...
		{
			if (strpos($path, 'css/') === 0 && strpos($path, '/admin/') === false)
			{
	...
	{
		if (self::$in_app && get_setting('ui_style') !== 'default') {
	...
	public static function is_output($output_filename, $template_filename)
	{
		...
		if (is_array($output_filename))
		{
			...
			{
				if (!in_array($val, self::$output_matchs[md5($template_filename)], true))
				{
					return false;
				}
			}
        ...
        if (in_array($output_filename, self::$output_matchs[md5($template_filename)], true))
        {
            return true;
        }
