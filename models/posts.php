...
class posts_class extends AWS_MODEL
{
    ...
    public $data_types = ['normal' => '普通', 'vote' => '投票', 'read_pay' => '付费查看', 'attach_pay' => '付费附件'];
	...
	public function process_explore_list_data($posts_index)
	{
		...
		foreach ($posts_index as $key => $data)
		{
			...
			// 主体数据类型标识
            if (isset($explore_list_data[$key]['data_type']) && $explore_list_data[$key]['data_type'] != 'normal' &&
                get_hook_info($explore_list_data[$key]['data_type'])['state'] == 1 &&
                (get_plugins_config($explore_list_data[$key]['data_type'])['status'] == 'yes' || get_plugins_config($explore_list_data[$key]['data_type'])['enable'] == 'Y')) {
                $explore_list_data[$key]['data_label'] = $this->data_types[$explore_list_data[$key]['data_type']];
            }
		}
