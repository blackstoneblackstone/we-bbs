...
class account_class extends AWS_MODEL
{
    ...
    public function get_user_info_by_email($email, $cache_result = true)
    {
        ...
        {
            return $this->get_user_info_by_uid($uid, true, $cache_result);
        }
    }
    ...
    public function setcookie_login($uid, $user_name, $password, $salt, $expire = null, $hash_password = true)
    {
        ...
        AWS_APP::session()->initialized = true;
        AWS_APP::session()->client_info['__CLIENT_UID'] = $uid;
        AWS_APP::session()->client_info['__CLIENT_USER_NAME'] = $user_name;
        AWS_APP::session()->client_info['__CLIENT_PASSWORD'] = $password;
        if($expire)
        {
            AWS_APP::session()->setExpirationSeconds(time() + $expire, 'client_info');
        }
    ...
    public function get_user_recommend_v2($uid, $limit = 10)
    {
        ...
        if (!$friends = $this->model('follow')->get_user_friends($uid,1, 100))
        {
