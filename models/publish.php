...
class publish_class extends AWS_MODEL
{
		...
		switch ($approval_item['type'])
		{
			case 'question':
				$question_id = $this->publish_question($approval_item['data']['question_content'], $approval_item['data']['question_detail'],
                    $approval_item['data']['category_id'], $approval_item['uid'], $approval_item['data']['topics'], $approval_item['data']['anonymous'],
                    $approval_item['data']['attach_access_key'], $approval_item['data']['ask_user_id'], $approval_item['data']['permission_create_topic'],null,0, $approval_item['data']['extend']);
			...
			case 'article':
				$article_id = $this->publish_article($approval_item['data']['title'],$approval_item['data']['article_img'], $approval_item['data']['message'],
                    $approval_item['uid'], $approval_item['data']['topics'], $approval_item['data']['category_id'], $approval_item['data']['column_id'],
                    $approval_item['data']['attach_access_key'], $approval_item['data']['permission_create_topic'], $approval_item['data']['extend']);
	...
	public function decline_publish($id)
	{
		...
		switch ($approval_item['type'])
		{
			...
			case 'article':
                ...
                // 审核拒绝,修改状态不直接删除记录
                $this->update('approval',array('status' => -1), 'id = ' . intval($approval_item['id']));
	...
	public function publish_answer($question_id, $answer_content, $uid, $anonymous = null, $attach_access_key = null, $auto_focus = true, $reply_to_openid = true)
	{
		...
		{
			$this->update_attach('answer', $answer_id, $attach_access_key);
		}
		...
		if ($weixin_user = $this->model('openid_weixin_weixin')->get_user_info_by_uid($question_info['published_uid']) AND $question_info['published_uid'] != $uid)
		{
			...
			if ($weixin_user_info['weixin_settings']['NEW_ANSWER'] != 'N')
			{
                ...
                if(get_setting('weixin_notify_type') == 'TEMPLATE')
                {
	                ...
	                $send_message = json_encode($send_message,JSON_UNESCAPED_UNICODE);
	...
	public function publish_question($question_content, $question_detail, $category_id, $uid, $topics = null, $anonymous = null, $attach_access_key = null, $ask_user_id = null, $create_topic = true, $from = null,$is_del = 0, $extend = [])
	{
		if ($question_id = $this->model('question')->save_question($question_content, $question_detail, $uid, $anonymous, null, $from,$is_del))
		{
            ...
            if(get_plugins_config('elasticsearch')['status']=='Y') {
                ...
                $question_info['type'] = 'questions';
                $question_info['search_id'] = $question_id;
                $question_info['uid'] = $uid;
                $this->model('elasticsearch')->index($question_info, 'question');
            }

            // 投票扩展更新
            if (get_hook_info('vote')['state'] == 1 && get_plugins_config('vote')['status'] == 'yes') {
                run_hook('vote_relation', array_merge(['vote_type' => 'question', 'item_id' => $question_id], $extend));
            }

            //问题发布后置钩子
            run_hook('publish_question_hook',['data' => $extend, 'action' => 'after','question_id' => $question_id]);
		}
		return $question_id;
	}

	public function publish_article($title,$logo_img, $message, $uid, $topics = null, $category_id = null,$column_id=null, $attach_access_key = null, $create_topic = true, $extend = [])
	{
		...
		)))
		{
			...
			if ($attach_access_key)
			{
				$this->update_attach('article', $article_id, $attach_access_key);
			}
            ...
            if(get_plugins_config('elasticsearch')['status']=='Y') {
                $article_info = $this->model('article')->get_article_info_by_id($article_id);
                $article_info['type']='articles';
                $article_info['search_id']=$article_id;
                $article_info['uid']=$uid;
                $this->model('elasticsearch')->index($article_info,'article');
            }

            // 投票扩展更新
            if (get_hook_info('vote')['state'] == 1 && get_plugins_config('vote')['status'] == 'yes') {
                run_hook('vote_relation', array_merge(['vote_type' => 'article', 'item_id' => $article_id], $extend));
            }

            // 文章发布后钩子
            run_hook('publish_article_hook',['data' => $extend, 'action' => 'after','article_id' => $article_id]);
		}
        //百度推送，新增记录
	...
	public function publish_article_comment($article_id, $message, $uid, $at_uid = null)
	{
		...
		if ($weixin_user = $this->model('openid_weixin_weixin')->get_user_info_by_uid($article_info['uid']) AND $article_info['uid'] != $uid)
		{
			...
			if ($weixin_user_info['weixin_settings']['NEW_ARTICLE_COMMENT'] != 'N')
			{
                ...
                if(get_setting('weixin_notify_type') == 'TEMPLATE')
                {
                    $template_info = $this->model('weixin')->fetch_row('weixin_template',"template_type = 'NEW_ARTICLE_ANSWER'");
	...
	public function add_attach($item_type, $file_name, $attach_access_key, $add_time, $file_location, $is_image = false, $extend = [])
	{
        $data = array(
		...
		if (!empty($extend)) $data = array_merge($data, $extend);

		return $this->insert('attach', $data);
	}
	...
	public function parse_attach_data($attach, $item_type, $size = null)
	{
		...
		foreach ($attach as $key => $data)
		{
			...
			$data['path'] = $data['outer_link'] ?: ($attach_path . $data['file_location']);
			$data['attachment'] = $data['outer_link'] ?: ($attach_url . $data['file_location']);
			$data['delete_id'] = AWS_APP::crypt()->encode(json_encode(['attach_id' => $data['id'], 'access_key' => $data['access_key']]));

			if ($data['outer_link']) {
                $data['url_path'] = $data['outer_link'];
            } else {
                if ($data['is_image'] == 1 AND $size)
                {
                    $data['thumb'] = $attach_url . AWS_APP::config()->get('image')->attachment_thumbnail[$size]['w'] . 'x' . AWS_APP::config()->get('image')->attachment_thumbnail[$size]['h'] . '_' . $data['file_location'];
                } else {
                    $data['url_path'] = $attach_url . $data['file_location'];
                }
            }

            $attach_list[$data['id']] = $data;
		}
	...
	{
		if ($approval_list = $this->fetch_page('approval', "`type` = '" . $this->quote($type) . "'  AND `status` = 0", 'time ASC', $page, $per_page))
		{
     ...
     * @param $table 'question|article'表
     * @return bool|mixed
     * @throws Zend_Exception
     */
    public function get_user_publish_num($user_id,$table)
    {
        ...
        if ($table === 'question'){
            $where_uid = 'published_uid = ' . $user_id;
        }else{
            $where_uid = 'uid = ' . $user_id;
        }

        $where = $where_uid . ' AND is_del = 0 AND ' .
            'add_time BETWEEN ' . $today_first_seconds . ' AND ' . $today_last_seconds;
