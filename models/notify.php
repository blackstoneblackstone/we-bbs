...
class notify_class extends AWS_MODEL
{
    ...
    const TYPE_REPORT = 150; // 新的举报通知管理员
	const TYPE_REPORT_APPROVED = 151; // 举报审核通过
	const TYPE_REPORT_REFUSED = 152; // 举报审核拒绝
    ...
    public function list_notification($recipient_uid, $read_status = 0, $limit = null)
    {
        ...
        foreach ($notify_list as $key => $val)
        {
            ...
            if ($read_status == 0 AND !empty($unread_extends[$val['model_type']][$val['source_id']]) AND count($unread_extends[$val['model_type']][$val['source_id']]) AND $this->notify_action_details[$val['action_type']]['combine'] == 1)
            {
        ...
        foreach ($notify_list as $key => $notify)
        {
            ...
            if($data['decline_reason']){
                $tmp_data['decline_reason'] = $data['decline_reason'];
	...
	public function format_extend_detail($extends, $user_infos)
    {
    ...
    public function get_notification_list($recipient_uid, $read_flag = null, $limit = null)
    {
        if (!$recipient_uid)
        {
            return false;
     ...
     * 获得用户未读合并通知
     */
    public function get_unread_notification($recipient_uid)
    {
    ...
    public function format_notification($data)
    {
        foreach ($data AS $key => $val)
        {
            ...
            else
            {
                ...
                {
                    case self::TYPE_REPORT_APPROVED:

                        $val['title'] = $data[$key]['title'] = $data[$key]['message'] = AWS_APP::lang()->_t('你提交的举报已被管理员审核').'  <a href="'.$data[$key]['url'].'" target="_blank">>>点击查看</a>';

                        break;

                    case self::TYPE_REPORT_REFUSED:
                        $val['title'] = $data[$key]['title'] =$data[$key]['message'] = AWS_APP::lang()->_t('你提交的举报已被管理员拒绝审核,拒绝理由：'.$data[$key]['decline_reason']).'  <a href="'.$data[$key]['url'].'" target="_blank">>>点击查看</a>';
                        break;
