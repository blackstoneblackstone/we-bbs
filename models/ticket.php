...
class ticket_class extends AWS_MODEL
{
    ...
    public function reply_ticket($ticket_id, $message, $uid, $attach_access_key = null)
    {
        ...
        if ($ticket_info['weibo_msg_id'])
        {
            $this->model('openid_weibo_weibo')->reply_answer_to_sina($ticket_info['question_id'], cjk_substr($message, 0, 110, 'UTF-8', '...'));
        }

        if ($ticket_info['received_email_id'])
        {
            $this->model('edm')->reply_answer_by_email($ticket_info['question_id'], html_entity_decode(nl2br(FORMAT::parse_bbcode($message))));
        }
    ...
    public function copy_attach($old_type, $new_type, $id, $uid)
    {
        ...
        foreach ($attachs AS $attach)
        {
            ...
            $array = explode('.', $attach['file_location']);
