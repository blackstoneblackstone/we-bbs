...
class question_class extends AWS_MODEL
{
	...
	public function update_question($question_id, $question_content, $question_detail, $uid, $verified = true, $modify_reason = null, $anonymous = null, $category_id = null)
	{
        ...
        if(get_plugins_config('elasticsearch')['status']=='Y') {
            ...
            $this->model('elasticsearch')->update($question_info, 'question');
        }
        //百度推送，更新记录
	...
	{
		if (!$uid OR !is_array($question_ids) OR count($question_ids) < 1)
		{
			return array();
		}
	...
	{
		if (!is_array($question_ids) OR count($question_ids) == 0)
		{
			return false;
		}
    ...
    public function clear_question($question_id)
    {
	    ...
	    ACTION_LOG::delete_action_history_old('associate_type = ' . ACTION_LOG::CATEGORY_QUESTION .  ' AND associate_id = ' . intval($question_id));	// 删除动作
	    ACTION_LOG::delete_action_history_old('associate_type = ' . ACTION_LOG::CATEGORY_QUESTION .  ' AND associate_action = ' . ACTION_LOG::ANSWER_QUESTION . ' AND associate_attached = ' . intval($question_id));	// 删除动作
	    ACTION_LOG::delete_action_history_old('associate_type = ' . ACTION_LOG::CATEGORY_TOPIC . ' AND associate_action = ' . ACTION_LOG::ADD_TOPIC . ' AND associate_attached = ' . intval($question_id));	// 删除动作
    ...
    public function get_focus_all($uid, $page = 1,$limit = 10,$post_type = 'all')
	{
		if($post_type == 'all' || $post_type == 'user')
		{
			...
			if($friend_uids)
			{
				$user_question_lists = $this->fetch_all('question', "published_uid IN(" . implode(',', $friend_uids).") and is_del =0 AND anonymous=0", 'add_time DESC');
		...
		if ($question_ids)
		{
			$question_lists = $this->fetch_all('question', "question_id IN(" . implode(',', array_unique($question_ids)) . ") and is_del =0 and anonymous=0", 'add_time DESC');
    ...
    public function get_by_answer_like($q, $limit = 10)
    {
        if(!is_array($q))
            $q=(array)$q;
        $where1 = "";
        $sql = "select answer_id,question_id,answer_content from ".$this->get_table('answer')." where is_del = 0 ".$where1." and answer_content like '%".$this->quote(implode('|', $q))."%'";
        return $this->query_all($sql,"$limit offset ".(AWS_APP::session()->answer_page * $limit));
    }
}
