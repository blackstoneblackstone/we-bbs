...
class project_class extends AWS_MODEL
{
    ...
    public function get_projects_list($category_id, $approved, $status = null, $page, $per_page, $order_by = 'add_time DESC')
    {
        ...
        return $this->fetch_page('project', implode(' AND ',$where), $order_by, $page, $per_page);
    }
    ...
    public function get_project_list_by_topic_ids($topic_ids, $order_by = 'add_time DESC', $page = 1, $per_page = 10)
    {
        ...
        $result_cache_key = 'project_list_by_topic_ids_' . implode('_', $topic_ids) . '_' . md5($order_by . $page . $per_page);

        $found_rows_cache_key = 'project_list_by_topic_ids_found_rows_' . implode('_', $topic_ids) . '_' . md5( $per_page );
    ...
    public function get_single_project_order_by_uid($uid, $project_id)
    {
        ...
        foreach ($order_list AS $key => $order_info)
        {
            ...
            {
                $tmp = $this->model('publish')->get_attach('product_order', $order_info['id'], null);

                if (!empty($tmp) && $attach=reset($tmp))
                {
    ...
    public function get_order_list($filter, $page, $per_page)
    {
        ...
        return $this->fetch_page('product_order', $where, 'id DESC', $page, $per_page);
    }
