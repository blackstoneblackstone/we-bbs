...
class main extends AWS_CONTROLLER
{
	...
	public function index_action()
	{
		...
		{
            $article_info['attachs'] = $this->model('publish')->get_attach('article_attach', $article_info['id']);
		...
		$article_info['message'] = nl2br(FORMAT::parse_imgs((FORMAT::parse_bbcode($article_info['message']))));
		...
		TPL::assign('reputation_topics', $this->model('people')->get_user_reputation_topic($article_info['user_info']['uid'], $article_info['user_info']['reputation'], 5));
		...
		//文章后置钩子
        run_hook('article_detail_hook',['data'=>$_GET,'action'=>'after','article_info'=>$article_info,'platform'=>'web']);
	...
	public function index_square_action()
	{
        ...
        if (!empty($article_list)) {
            $data_types = $this->model('posts')->data_types;
            foreach ($article_list as $key => $val) {
                if (isset($val['data_type']) && $val['data_type'] != 'normal' &&
                    get_hook_info($val['data_type'])['state'] == 1 &&
                    (get_plugins_config($val['data_type'])['status'] == 'yes' || get_plugins_config($val['data_type'])['enable'] == 'Y')) {
                    $article_list[$key]['data_label'] = $data_types[$val['data_type']];
                }
            }
        }
		...
		//钩子
        run_hook('action_article_main',['list'=>$article_list]);
