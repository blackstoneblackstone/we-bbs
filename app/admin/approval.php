...
class approval extends AWS_ADMIN_CONTROLLER
{
	public function list_action()
	{
		...
		TPL::assign('answer_count', $this->model('publish')->count('approval', "type = 'answer' AND status = 0"));

		TPL::assign('question_count', $this->model('publish')->count('approval', "type = 'question' AND status = 0"));

		TPL::assign('article_count', $this->model('publish')->count('approval', "type = 'article' AND status = 0"));

		TPL::assign('article_comment_count', $this->model('publish')->count('approval', "type = 'article_comment' AND status = 0"));
	...
	public function preview_action()
	{
		...
		switch ($approval_item['type'])
		{
			case 'question':
				...
				$approval_item['topics'] = htmlspecialchars(is_array($approval_item['data']['topics']) ? implode(',', $approval_item['data']['topics']) : $approval_item['data']['topics']);
