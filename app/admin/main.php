    ...
    public function index_action()
    {
        ...
        if (!defined('IN_SAE'))
        {
            $writable_check = array(
                'cache' => is_really_writable(PUB_PATH . 'cache/'),
                'tmp' => is_really_writable(PUB_PATH . './tmp/'),
        ...
        TPL::assign('approval_question_count', $this->model('publish')->count('approval', "status=0 AND type = 'question'"));
        TPL::assign('approval_answer_count', $this->model('publish')->count('approval', "status=0 AND type = 'answer'"));
