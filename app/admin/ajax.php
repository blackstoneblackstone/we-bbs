...
class ajax extends AWS_ADMIN_CONTROLLER
{
    ...
    public function save_settings_action()
    {
        ...
        if($_FILES['watermark_image']=='' && get_setting('watermark_image'))
        {
            unset($_POST['watermark_image']);
        }else{
            if ($_FILES['watermark_image']['name'])
            {
                AWS_APP::upload()->initialize(array(
                    'allowed_types' => 'jpg,jpeg,png,gif',
                    'upload_path' => get_setting('upload_dir') . '/water/',
                    'is_image' => TRUE,
                    'max_size' => get_setting('upload_avatar_size_limit'),
                    'encrypt_name' => FALSE,
                    'file_name' =>'water_logo'
                ))->do_upload('watermark_image');
                if (AWS_APP::upload()->get_error())
                {
                    switch (AWS_APP::upload()->get_error())
                    {
                ...
                $_POST['watermark_image'] = get_setting('upload_url') . '/water/' . $upload_data['file_name'];
            }
        }
        ...
        if($cache_type =='memcache' && !extension_loaded('memcache'))
        {
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('您还未开启Memcache拓展')));
        }
    ...
    public function report_manage_action()
    {
        ...
        }else if($_POST['action_type']=='decline')
        {
            $this->model('question')->update_report(intval($_POST['report_ids']), array(
                'status' => 2,
                'decline_reason'=>htmlspecialchars(trim($_POST['reason']))
            ));

            //拒绝审核
            $report_info = AWS_APP::model()->fetch_row('report','id='.intval($_POST['report_ids']));
            $this->model('notify')->send($this->user_id, $report_info['uid'], notify_class::TYPE_REPORT_REFUSED,notify_class::CATEGORY_QUESTION, $report_info['id'], array(
                'type' => $report_info['type'],
                'decline_reason'=>$report_info['decline_reason'],
                'url'=>$report_info['url'],
            ));
        }
        else if ($_POST['action_type'] == 'handle')
        {
            foreach ($_POST['report_ids'] as $val)
            {
                ...
                //通过审核
                $report_info = AWS_APP::model()->fetch_row('report','id='.intval($val));
                $this->model('notify')->send($this->user_id, $report_info['uid'], notify_class::TYPE_REPORT_APPROVED,notify_class::CATEGORY_QUESTION, $report_info['id'], array(
                    'type' => $report_info['type'],
                    'decline_reason'=>$report_info['decline_reason'],
                    'url'=>$report_info['url'],
                ));
            ...
            case 'decline':
                foreach ($_POST['approval_uids'] AS $approval_uid)
                {
                    if ($user_info = $this->model('account')->get_user_info_by_uid($approval_uid))
                    {
                        ...
                        $this->model('system')->remove_user_by_uid($approval_uid,htmlspecialchars($_POST['reason']));
                        //日志记录
