...
class plugin extends AWS_ADMIN_CONTROLLER
{
    ...
    public function save_config_action()
    {
        ...
        if (in_array($plugin_name, ['read_pay'])) {
            if ($_POST['config']['pay_type'] == 'pay' && !get_hook_info('pay')) H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('请先安装付费插件或购买授权版')));
        }

        // 插件配置前钩子
        hook($plugin_name, 'config_before_hook', $_POST);
    ...
    public function upgrade_action()
    {
        ...
        $plugin_dir = $this->model('plugin')->get_plugin_dir($plugin_name);
    ...
    public function save_plugin_action()
    {
        ...
        $author = $_POST['author'] ? htmlspecialchars($_POST['author']) : '插件开发者';
        $version = $_POST['version'] ? htmlspecialchars($_POST['version']) : '1.0.0';
