...
class weixin extends AWS_MOBILE_CONTROLLER
{
		...
		if ($_GET['code'] AND get_setting('weixin_app_id') AND get_setting('weixin_app_secret'))
		{
			...
			if ($access_token = $this->model('openid_weixin_weixin')->get_sns_access_token_by_authorization_code($_GET['code']))
			{
				...
				if ($weixin_user = $this->model('openid_weixin_weixin')->get_user_info_by_openid($access_token['openid']))
				{
                    ...
                    AWS_APP::session()->client_info['__CLIENT_PASSWORD'] = $user_info['password'];
	...
	public function authorization_action()
	{
		...
		else if ($_GET['code'] OR $_GET['state'] == 'OAUTH')
		{
			...
			if ($access_token && !is_array($access_token) && !is_object($access_token))
			{
				if ($access_token['errcode'])
				{
				...
				if (get_setting('register_type') == 'weixin')
				{
					...
					if($wenixn_info = $this->model('openid_weixin_weixin')->get_user_info_by_openid($access_token['openid'])){
						if($uf = $this->model('account')->get_user_info_by_uid($wenixn_info['uid'])){
                            Zend_Session::regenerateId ();
                            AWS_APP::session()->initialized = true;
                            AWS_APP::session()->client_info['__CLIENT_UID'] = $uf['uid'];
                            AWS_APP::session()->client_info['__CLIENT_USER_NAME'] = $uf['user_name'];
                            AWS_APP::session()->client_info['__CLIENT_PASSWORD'] = $uf['password'];
