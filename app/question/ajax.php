...
class ajax extends AWS_CONTROLLER
{
	...
	{
        run_hook('remove_question_action_hook', ['question_id' => $_POST['question_id'], 'action' => 'before']); // 删除问题钩子
		...
		if ($question_info)
		{
            ...
            run_hook('remove_question_action_hook', ['question_info' => $question_info, 'action' => 'after']); // 删除问题钩子
		}
    ...
    public function set_top_question_action()
    {
        ...
        $set_top_num = get_setting('set_top_num') ?: 3;
        if($this->model('posts')->select_set_top_num()[0]['count(*)']>=$set_top_num && $_POST['action'] == 'set'){
            H::ajax_json_output(AWS_APP::RSM(null, '-1', AWS_APP::lang()->_t('对不起, 置顶最多不超过'.$set_top_num.'条')));
        }
        switch ($_POST['action'])
        {
            case 'set':
    ...
    {
        run_hook('remove_answer_action_hook', ['answer_id' => $_POST['answer_id'], 'action' => 'before']); // 删除回答钩子
        if (! $answer_info = $this->model('answer')->get_answer_by_id($_POST['answer_id']))
        {
            H::ajax_json_output(AWS_APP::RSM(null, '-1', AWS_APP::lang()->_t('答案不存在')));
        }
        ...
        $this->model('question')->save_last_answer($answer_info['question_id']);
        run_hook('remove_answer_action_hook', ['answer_id' => $_POST['answer_id'], 'action' => 'after']); // 删除回答钩子
