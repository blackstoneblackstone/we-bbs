...
$(function()
{
		...
		if(G_DRAFT_ENABLED == 'Y'){
			if (G_ADVANCED_EDITOR_ENABLE == 'Y'){
				...
				$('#wmd-input').bind('blur',function(){
					var  article_textarea = $('#wmd-input').val();
					if(article_textarea != ''){
						$.post(G_BASE_URL + '/account/ajax/save_draft/item_id-'+QUESTION_ID+'__type-answer', 'message=' + article_textarea, function (result) {
							$('#question_detail_message').html(result.err + ' <a href="#" onclick="$(\'textarea#advanced_editor\').attr(\'value\', \'\'); AWS.User.delete_draft('+ITEM_ID+', \'' + PUBLISH_TYPE + '\'); $(this).parent().html(\' \'); return false;">' + _t('删除草稿') + '</a>');
						}, 'json');
					}
				});
	...
	$('#saveDraft').click(function() {
		...
		if(article_textarea != ''){
			$.post(G_BASE_URL + '/account/ajax/save_draft/item_id-'+QUESTION_ID+'__type-answer', 'message=' + article_textarea, function (result) {
					$('#question_detail_message').html(result.err + ' <a href="#" onclick="$(\'textarea#advanced_editor\').attr(\'value\', \'\'); AWS.User.delete_draft('+QUESTION_ID+', \'answer\'); $(this).parent().html(\' \'); return false;">' + _t('删除草稿') + '</a>');
			}, 'json');
		}
	});
